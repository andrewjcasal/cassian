...
export const fetchAllCalendarData = async (userId: string) => {
  ...
  try {
    ...
    const [habitsResult, sessionsResult, projectsResult, meetingsResult, tasksDailyLogsResult, tasksResult, settingsResult, calendarNotesResult, habitNotesResult, categoryBuffersResult, projectActivityResult] = await Promise.allSettled([
    ...
    return { habits, sessions, projects, meetings, tasksDailyLogs, tasks, settings, calendarNotes, habitNotes, categoryBuffers, projectActivity }
  } catch (error) {
    console.error('Critical error fetching calendar data:', error)
    // Return minimal data to prevent complete failure
    return { habits: [], sessions: [], projects: [], meetings: [], tasksDailyLogs: [], tasks: [], settings: null, calendarNotes: [], habitNotes: [], categoryBuffers: [], projectActivity: [] }
  }
}
